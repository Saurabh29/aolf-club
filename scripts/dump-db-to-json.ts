...
async function dumpTableToJson() {
  ...
  try {
    ...
    do {
      ...
      const response: ScanCommandOutput = (await docClient.send(command)) as ScanCommandOutput;

      if (response.Items && response.Items.length > 0) {
        allItems.push(...response.Items);
        console.log(`  Retrieved ${response.Items.length} items (total: ${allItems.length})`);
      }
